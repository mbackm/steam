...
// @match        https://hentaiverse.org/?s=Bazaar&ss=ml*
...
function reload_data(){
    ...
    if (ml_load){
        ...
        CHECK_MON_HUNGRY = ml_data["CHECK_MON_HUNGRY"];
...
function checkState(doc, param, max, type) {
    var stat = doc.getElementById("monster_outer");
    if (stat) {
		...
		*/
        ...
        var tables = doc.querySelectorAll('#monsterstats_left table');
        ...
        if("PRI" === type){
            ...
            console.log('Upgrade.pa:'+Upgrade.pa);
            if((max-0) === nowValue*1){
    ...
    function checkChaos(table, list) {
        var array = {};
        for (var i=0; i<table.rows.length; i++) {
            var count = table.rows[i].cells[1].querySelectorAll("div.mcu2").length;
...
function upStatPri(vMainData, vMaxSetting, vSlot){
    ...
    document.getElementById('monster_outer').insertBefore(vDiv, document.getElementById('monsterstats_left'));
    console.log('hhhhhhhhhhhhhhhh');
    for(var j=0;j<vMainValue.length;j++){
        var vValue = vMainValue[j];
        var img = document.querySelector('img[onclick*="\''+vValue+'\'"]');
        ...
        if(img){
			...
			*/
            ...
            var req = {
                type: "POST",	
                tag: [],	
                url: "https://hentaiverse.org/?s=Bazaar&ss=ml&slot=", 
                param: "", 
                callback: checkState,
                repeat: 1, 
                elm: null, 
...
function genLetGoMonBtn(){
    ...
    if(document.querySelectorAll("#monsterstats_top div.mcr").length < 1) return;
    ...
    linkx.addEventListener('click', function() {
        ...
        var nowMon = document.querySelector('#monster_head div.msl div').textContent;
    ...
    var _div_main = $element("div",document.getElementById('csp'),[".ml_st"]);
    _div_main.appendChild(linkx);

    var _div_setting = $element("div",document.getElementById('csp'),[".ml_mst"]);
    ...
    var ttr2 = $element("tr",mRParaTable);
    var tdd12 = $element("td",ttr2);
    tdd12.innerHTML = "CHECK MON HUNGRY";
    var tdd22 = $element("td",ttr2);

    var chkNM2 = $element("input",tdd22,{type:"checkbox",value:"CHECK_MON_HUNGRY"},function(){
        data[this.value] = this.checked;
        localStorage.setItem("MS_monsterSetting", JSON.stringify(data));
    });

    if(data["CHECK_MON_HUNGRY"]) chkNM2.checked = data["CHECK_MON_HUNGRY"];
...
if(letGo === "false"){
    ...
    localStorage.setItem("MS_monsterLetGo", false);
}else if( document.querySelector('#monster_head div.msl div')){
    ...
    var bolChkGoHungry = true;
    if(CHECK_MON_HUNGRY){
        var hgNow = document.querySelectorAll('div.msn img')[1].width;
        var perCenNow = hgNow*100/120;

        if(perCenNow > 50){
            bolChkGoHungry = false;
        }
    }
